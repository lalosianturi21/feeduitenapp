...
class App extends React.Component {
  ...
  tambahItem(objek) {
    ...
    let nominalUang = dataUangIN.length > 0 ? dataUangIN.map((item) => item.nominal) : []
    let jumlahUangIn = nominalUang.reduce((total, num) => total + num, 0 )
    let dataUangOUT = newData.filter((item) => item.category === 'OUT')
    let nominalUangOUT = dataUangOUT.length > 0 ? dataUangOUT.map((item) => item.nominal) : []
        ...
        <div className="container py-5">
          <div className='row'>
            <div className='col-12 text-center'>
              ...
              <span className='title-sm'>Sisa uang kamu tersisa {this.state.presentaseUang}% lagi</span>
            </div>
          </div>
          ...
          <div className='row mt-5'>
            <div className='col-12 d-flex justify-content-between align-items-center'>
              ...
              <div className='wrapper-button'>
                <ModalCreate action={this.tambahItem} category='IN' variant='button btn-ungu px-3 py-2 me-2' text='Pemasukan' icon='bi bi-plus-circle-fill' modalheading='Tambhakan Pemasukan' />
                <ModalCreate action={this.tambahItem} category='OUT' variant='button btn-pink px-3 py-2 ' text='Pengeluaran' icon='bi bi-dash-circle-fill' modalheading='Tambhakan Pengeluaran' />
              </div>
            </div>
          </div>
